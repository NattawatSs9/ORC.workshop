    ...
    public class OrderService
    {
        ...
        public DataTable GetOrder()
        {
            try
            {
                ...
                string query = @"SELECT b.booking_id , b.booking_code, q.quotation_name, b.delivery_datetime, b.delivery_date, c.company_name, s.site_name, pl.plant_name, p.product_name, b.quantity, b.contact_name, b.tel, b.casting_method, b.status  FROM booking_order b
        ...
        public string AddOrder(OrderModel orderModel)
        {
            ...
            {
                string query =
                    @"INSERT INTO booking_order(delivery_datetime, delivery_date, casting_method, tel, contact_name, quantity, booking_code, more_detail, company_id, plant_id, site_id, quotation_id, product_id, status)
                      VALUES (@delivery_datetime, @delivery_date, @casting_method, @tel, @contact_name, @quantity, @booking_code, @more_detail, @company_id, @plant_id, @site_id, @quotation_id, @product_id, @status)";
                NpgsqlDataReader reader;
                using (NpgsqlConnection connection = new NpgsqlConnection(_sqlDataSource))
                {
                    connection.Open();
